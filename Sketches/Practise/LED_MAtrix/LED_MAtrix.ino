...
void loop() {
  ...
  staticImage(smilingFace);
  scrollingImage(data);
  // Display the dynamic patterns of numbers and letters

}

void staticImage(int img[]){
    for (int j = 0; j < 500; j++ ) {  // repeat 500 times
    int cols = 0x01; // Assign 0x01(binary 00000001) to the variable, which represents the first column is selected.
    for (int i = 0; i < 8; i++) {   // display 8 column data by scaning
      matrixColsVal(cols);          // select this column
      matrixRowsVal(img[i]);// display the data in this column
